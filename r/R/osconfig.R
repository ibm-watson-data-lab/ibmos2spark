  ...
  methods=list(initialize =
    function( sparkcontext, name, auth_url, region, username, password,public=FALSE){
      ...
      if ( grepl('_',name)){
        ...
        hConf = callJMethod(sparkcontext, "hadoopConfiguration")
        callJMethod(hConf, "set", paste(prefix, "auth.url", sep='.'),auth_url)
        callJMethod(hConf, "set", paste(prefix, "auth.endpoint.prefix", sep='.'), "endpoints")
        callJMethod(hConf, "set", paste(prefix, "tenant", sep='.'), username)
        callJMethod(hConf, "set", paste(prefix, "username", sep='.'), username)
        callJMethod(hConf, "set", paste(prefix, "password", sep='.'), password)
        callJMethod(hConf, "set", paste(prefix, "apikey", sep='.'), password)
        callJMethod(hConf, "set", paste(prefix, "use.get.auth", sep='.'), "true")
        callJMethod(hConf, "set", paste(prefix, "apikey", sep='.'), password)
        callJMethod(hConf, "set", paste(prefix, "http.port", sep='.'), "8080")
        callJMethod(hConf, "set", paste(prefix, "region", sep='.'), region)
        invisible(callJMethod(hConf, "setBoolean", paste(prefix, "public", sep='.'), public))
        invisible(callJMethod(hConf, "setBoolean", paste(prefix, "location-aware", sep='.'), FALSE))},
...
softlayer2d <- setRefClass("softlayer2d",
  ...
  methods=list(initialize =
    function( sparkcontext, name, auth_url, tenant, username, password,public=FALSE,
        ...
        hConf = callJMethod(sparkcontext, "hadoopConfiguration")
        callJMethod(hConf, "set", "fs.swift2d.impl", swift2d_driver)
        callJMethod(hConf, "set", paste(prefix, "auth.url", sep='.'), auth_url)
        callJMethod(hConf, "set", paste(prefix, "username", sep='.'), username)
        callJMethod(hConf, "set", paste(prefix, "tenant", sep='.'), tenant)
        callJMethod(hConf, "set", paste(prefix, "auth.endpoint.prefix", sep='.'), "endpoints")
        callJMethod(hConf, "set", paste(prefix, "auth.method", sep='.'), "swiftauth")
        callJMethod(hConf, "set", paste(prefix, "http.port", sep='.'), "8080")
        callJMethod(hConf, "set", paste(prefix, "apikey", sep='.'), password)
        invisible(callJMethod(hConf, "setBoolean", paste(prefix, "public", sep='.'), public))
        callJMethod(hConf, "set", paste(prefix, "use.get.auth", sep='.'), "true")
  ...
  methods=list(initialize =
    function(..., sparkcontext, name=NULL, credentials,public=FALSE){
      ...
      if(class(tenant)=="try-error")  tenant = credentials['projectId'][[1]]
        ...
        prefix = paste("fs.swift.service" , name, sep =".")
        hConf = callJMethod(sparkcontext, "hadoopConfiguration")
        callJMethod(hConf, "set", paste(prefix, "auth.url", sep='.'), paste(credentials['auth_url'][[1]],"/v3/auth/tokens",sep=""))
        callJMethod(hConf, "set", paste(prefix, "auth.endpoint.prefix", sep='.'), "endpoints")
        callJMethod(hConf, "set", paste(prefix, "tenant", sep='.'), tenant)
        callJMethod(hConf, "set", paste(prefix, "username", sep='.'), user_id)
        callJMethod(hConf, "set", paste(prefix, "password", sep='.'), credentials['password'][[1]])
        callJMethod(hConf, "set", paste(prefix, "region", sep='.'), credentials['region'][[1]])
        invisible(callJMethod(hConf, "setBoolean", paste(prefix, "public", sep='.'), credentials['public'][[1]]))},
  ...
  methods=list(initialize =
    function(..., sparkcontext, name=NULL, credentials,
      ...
      if(class(tenant)=="try-error")  tenant = credentials['projectId'][[1]]
        ...
        callJMethod(hConf, "set", paste(prefix, "username", sep='.'), user_id)
        callJMethod(hConf, "set", paste(prefix, "password", sep='.'), credentials['password'][[1]])
        callJMethod(hConf, "set", paste(prefix, "auth.method", sep='.'), "keystoneV3")
        callJMethod(hConf, "set", paste(prefix, "region", sep='.'), credentials['region'][[1]])
        invisible(callJMethod(hConf, "setBoolean", paste(prefix, "public", sep='.'), credentials['public'][[1]]))
        #invisible(callJMethod(hConf, "setInt", paste(prefix, "http.port", sep='.'), 8080))
          },
